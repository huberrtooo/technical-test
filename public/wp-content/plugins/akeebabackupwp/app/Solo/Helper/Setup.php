...
abstract class Setup
{
	...
	public static function databaseTypesSelect($selected = '', $name = 'driver')
	{
		...
		$connectors = array_filter($connectors, function (?string $driverName) {
	...
	public static function timezoneSelect($selected = '', $name = 'timezone', $includeDefaults = false, $disabled = false)
	{
		...
		if ($includeDefaults)
		{
			...
			if (defined('WPINC'))
			{
				$defaultGroup[] = Select::option('AKEEBA/DEFAULT', Text::_('COM_AKEEBA_CONFIG_FORCEDBACKUPTZ_DEFAULT_WORDPRESS'));
			}
			else
			{
				$defaultGroup[] = Select::option('AKEEBA/DEFAULT', Text::_('COM_AKEEBA_CONFIG_FORCEDBACKUPTZ_DEFAULT'));
			}
